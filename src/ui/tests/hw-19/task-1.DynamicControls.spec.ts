...
test.describe("[UI] Dynamic Controls", () => {
  test("Should auto-wait for dynamic content to appear", async ({ page }) => {
    ...
    const elTitleH4 = page.getByRole("heading", { name: "Dynamic Controls" });
    const paragraphText = page.locator("div.example > p");
    const subheaderText = page.getByRole("heading", { name: "Remove/add" });

    await expect(elTitleH4).toHaveText("Dynamic Controls");
    ...
    await expect(subheaderText).toHaveText("Remove/add");
